...
function botActions(utils) {
    ...
    if (/good (girl|bot)/gi.test(utils.getMessage())) {
        ...
        utils.bot.sendChat("@" + utils.getUserUsername() + " http://i.imgur.com/pWUhSoS.png");
        utils.reply.stop();
        return;
    }
}
    ...
    fs.stat(banData, function (err, stat) {
        ...
        for (var key in phrases) {
            if (typeof phrases[key] != 'function' && utils.getUserRole() == null) {
                var phrasesList;
                if(Array.isArray(phrases[key].phrase)) phrasesList = phrases[key].phrase;
                else phrasesList = [ phrases[key].phrase ];
                for(var i = 0; i < phrasesList.length; i++) {
                    if (utils.getMessage().toLowerCase().indexOf(phrasesList[i].toLowerCase()) != -1) {
...
function cleanChatImageTimeout(utils) {
    ...
    if (re.test(utils.getMessage().toLowerCase())) {
        ...
        if (utils.settingsManager.getImgTime() >= 0) {
            if (utils.getUserId() == utils.bot.getSelf().id) {
                setTimeout(function () {
                    utils.bot.moderateDeleteChat(utils.getId());
