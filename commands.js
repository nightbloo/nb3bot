            ...
            function (utils) {
                    ...
                    msg = '@' + utils.getUserUsername() + ' ~Your lucky number is ' + rnd + '~: ';
                {
                    ...
                    if (queueSpot >= 0) {
                        if (rnd === 373) {
                            if (queueSpot !== 0) {
                                utils.bot.sendChat(msg + 'Wow, 1 in 1000! You\'ll be moved to the *top* of the queue.');
                                utils.bot.moderateMoveDJ(utils.getUserId(), 0);
                            } else {
                                utils.bot.sendChat(msg + 'Well, this is interesting. You were going to be *first* in queue, but you already are!');
                            }
                            return;
                        } else if (rnd >= 634 && rnd < 639) {
                            var queueLength = utils.bot.getQueue().length - 1;
                            if (queueSpot !== queueLength) {
                                utils.bot.sendChat(msg + 'In peace you must rest :nb3RIP: , 5 in 1000. You\'ll be moved to the *bottom* of the queue.');
                                utils.bot.moderateMoveDJ(utils.getUserId(), queueLength);
                            } else {
                                utils.bot.sendChat(msg + 'Há!, saved but not. You were going to be *last* in queue, but you already are!');
                            }
                            return;
                        }
                    } else if (rnd === 373 || (rnd >= 634 && rnd < 639)) {
                        utils.bot.sendChat(msg + 'Uh, but you\'re not in queue. Nothing happens :x');
                        return;
                    }
                }
                utils.redisManager.getProps(utils.getUserId(), function (propsCount) {
                    ...
                    function changeProps(dec) {
                        ...
                        if (dec) {
                            msg += 'You lose 1 prop (now you have ' + result + ' prop' + (result !== 1 ? 's' : '') + ') :(.';
                            utils.redisManager.decProps(utils.getUserId());
                        } else {
                    ...
                    if (propsCount > 0) {
                        if (rnd >= 0 && rnd < 10) {
                            var win = Math.dice(2) === 1,
                                perc = Math.dice(1, 11),
                                change = Math.floor(propsCount * (perc / 10));

                            if (win) {
                                msg += 'Awesome! You get ' + perc + '% more props of what you had (' + change + ' prop' + (change !== 1 ? 's' : '') + ')';
                                utils.redisManager.incPropsBy(utils.getUserId(), change);
                            } else {
                                msg += 'Oh that\'s bad, you lose ' + perc + '% of your props (' + change + ' prop' + (change !== 1 ? 's' : '') + ')';
                                utils.redisManager.decPropsBy(utils.getUserId(), change);
                            }
                        } else {
                            changeProps(!onePropChangeType);
                        }
                    } else {
                        if (onePropChangeType) {
                            changeProps(false);
                        } else {
                            msg += 'Uh, but you don\'t have any props. Nothing happens :x';
                        }
                    }
